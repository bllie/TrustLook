...
public class AppDetailActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final String pkgName = getIntent().getStringExtra(
	...
	public void launchAppOPDialog() {
		...
		/*
		 * dialog.setButton(AlertDialog.BUTTON_NEGATIVE, "Launch", new
		 * DialogInterface.OnClickListener() { public void
		 * onClick(DialogInterface dialog, int id) { // start the app by
		 * invoking its launch intent Intent i =
		 * getPackageManager().getLaunchIntentForPackage
		 * (currentApp.getPackageName()); try { if (i != null) {
		 * startActivity(i); } else { i = new
		 * Intent(currentApp.getPackageName()); startActivity(i); } } catch
	...
	private class UploadApkTask extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
			...
			return PkgUtils.uploadTrustLook(AppListService.getInstance()
					.getDeviceId(), apkFile, currentApp.getMd5());
		}
